"""
This file was taken from https://github.com/AppointmentGuru/AuthenticationGuru/blob/master/kong_oauth/resource.py
...
class Response(object):
    ...
    def __init__(self, response, many=False):
        ...
        try:
            ...
            '''Could not parse response to ojbect'''
            pass
...
class Resource(object):
    ...
    def __init__(self, interface = {}):
        ...
        self.auth = getattr(self.api_class, 'auth')
    ...
    def call(self, caller, keys, data={}, params = {}, headers={}, *args, **kwargs):
        ...
        if self.auth and kwargs.get('auth') is None:
    ...
    def create_or_update(self, data, search_keys, search_params, create_keys, count_field='count', id_field='id'):
        assert False, 'Not yet implemented'
        result = self.list(search_keys, search_params)
        js = result.response.json()
        count = js.get(count_field, None)
        assert count is not None, \
            'Could not establish a count value. Are you using the correct count field? Result was: {}'.format(js)
